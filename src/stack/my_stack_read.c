...
static struct ExecResult MY_STACK_Read__Read_file_length(
...
static struct ExecResult MY_STACK_Read__Process_data(
    FILE* file, rec_type* file_desc, unsigned int length) {
...
static struct ExecResult MY_STACK_Read__Read_file_desc(
    FILE* file, rec_type** file_desc, unsigned int length) {

    size_t fread_result = fread(
        *file_desc, sizeof(*file_desc[0]), (size_t)length + 1, file);
