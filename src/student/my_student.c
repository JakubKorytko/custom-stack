...
void *MY_STUDENT_Init(char * surname, int rok, enum FIELDS_OF_STUDY field_of_study) {
	...
	struct MY_STUDENT* pdata = (struct MY_STUDENT*)malloc(sizeof(struct MY_STUDENT));

	if (pdata) {

		if (surname) {
			pdata->student_surname = (char*)malloc((strlen(surname) + 1) * sizeof(char));
			if (pdata->student_surname) {
				strncpy_s(pdata->student_surname, strlen(surname) + 1, surname, strlen(surname));
			}
		}
		else {
...
void* MY_STUDENT_SearchData() {
	...
	struct MY_STUDENT* searchDat = (struct MY_STUDENT*)malloc(sizeof(struct MY_STUDENT));
