...
_invalid_parameter_handler setInvalidParameterHandler() {
  ...
  old_param_hadler = _set_invalid_parameter_handler(&myInvalidParameterHandler);
...
struct ExecResult error(int errorCode, char *file, unsigned int line) {
  struct ExecResult res = {.errorCode = errorCode, .line = line, .file = file};

  return res;
}

struct ExecResult success() {
  struct ExecResult res = {.errorCode = -1, .line = 0, .file = NULL};
